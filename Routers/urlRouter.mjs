...
router.post("/shorten/auth", isAuthenticated, async (req, res) => {
	...
	try {
		...
		const qrCode = await QRCode.toDataURL(
			`https://url-shortener-api-rj6k.onrender.com/url/${shortUrl}`
		);
...
router.post("/shorten", async (req, res) => {
	...
	if (alreadyExist) {
		const qrCode = await QRCode.toDataURL(
			`https://url-shortener-api-rj6k.onrender.com/url/${alreadyExist.shortUrl}`
		);
		return res.json({
			msg: "URL already shortened",
			shortUrl: alreadyExist.shortUrl,
	...
	try {
		...
		res.json({
			...
			urls: urls.map((u) => ({
				...
				shortUrl: `https://url-shortener-api-rj6k.onrender.com/url/${u.shortUrl}`,
